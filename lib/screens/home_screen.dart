...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  bool _ipExpanded = false;  // dropdown state
  ...
  Future<String?> _getPrivateIp() async {
    try {
      ...
      // Fallback: interface pertama bukan loopback
      for (final iface in interfaces) {
        for (final addr in iface.addresses) {
          if (!addr.isLoopback) return addr.address;
        }
  ...
  // ── IP Public (fetch dari service eksternal) ──────────────────
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Row(children: [
          Icon(Icons.error_outline, color: Colors.red), SizedBox(width: 8), Text('Gagal'),
        ]),
        content: SingleChildScrollView(
          child: Text(msg, style: const TextStyle(fontFamily: 'monospace', fontSize: 13))),
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Text(
                    isActive ? 'Server Aktif' : 'Server Dijeda',
                    style: TextStyle(
              ...
              ListTile(
                ...
                title: Text(isActive ? 'Pause' : 'Resume',
                  style: TextStyle(fontWeight: FontWeight.w500, color: isActive ? Colors.orange : Colors.green)),
                subtitle: Text(
              ...
              ListTile(
                ...
                title: const Text('Stop', style: TextStyle(fontWeight: FontWeight.w500, color: Colors.red)),
                subtitle: const Text('Hentikan server sepenuhnya', style: TextStyle(fontSize: 12)),
                onTap: () {
                  Navigator.pop(ctx);
                  _service.stop();
                  ForegroundSvc.stop();
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  AnimatedContainer(
                    ...
                    child: Center(
                      child: ScaleTransition(
                        scale: state == ServerState.active ? _pulseAnim : const AlwaysStoppedAnimation(1.0),
                        child: GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            child: Icon(_buttonIcon(state), size: 60, color: Colors.white),
                          ),
                        ),
                      ),
                    ),
                  ...
                  AnimatedSwitcher(
                    ...
                    child: Text(_stateLabel(state),
                      key: ValueKey(state),
                      style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: _buttonColor(state))),
                  ),
                  const SizedBox(height: 6),
                  Text(
                    state == ServerState.idle ? 'Tap untuk memulai server' : 'Tap untuk kontrol server',
                    style: TextStyle(fontSize: 13, color: Colors.grey.shade500)),
                ]),
              ),
            ),

            // ── IP pill + dropdown — di luar Expanded ────────────
            if (isRunning)
              _buildIpSection(context),
            Container(
              ...
              child: ListTile(
                leading: const Icon(Icons.folder_rounded, color: Colors.amber, size: 28),
                title: Text(_selectedFolder, style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis),
                subtitle: const Text('Folder yang akan di-serve', style: TextStyle(fontSize: 11)),
                trailing: TextButton(onPressed: isRunning ? null : _pickFolder, child: const Text('Ganti')),
              ),
            ),
          ]),
        ),
  ...
  Widget _buildIpSection(BuildContext context) {
    ...
    // Teks yang ditampilkan di pill: IP private kalau sudah ada, fallback placeholder
          ...
          AnimatedCrossFade(
            ...
            secondChild: Container(
              ...
              child: Column(children: [
                ...
                _ipRow(
                  context: context, label: 'Loopback',
                ...
                _ipRow(
                  context: context, label: 'Local (WiFi)',
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          if (loading)
            ...
            Text('Tidak tersedia', style: TextStyle(fontSize: 11, color: Colors.grey.shade400)),
        ]),
      ),
    );
  }
